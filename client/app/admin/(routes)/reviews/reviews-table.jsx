...
import { Edit } from "@/node_modules/lucide-react";
    ...
    <Table>
      <TableCaption>
        {!Array.isArray(reviews) ||
          (reviews.length === 0 && (
            <h1 className="py-6 text-muted-foreground">No reviews found</h1>
          ))}
      </TableCaption>
      <TableHeader>
        <TableRow>
      ...
      <TableBody>
        {reviews.map((review) => (
          <TableRow key={review.ID}>
            ...
            <TableCell className="w-max break-words">{review.role}</TableCell>
            <TableCell className="text-right break-all">
              {review.review}{" "}
              <Dialog>
                <DialogTrigger asChild>
                  <Button variant="outline" className="mx-2 p-2">
                    <Edit />
                  </Button>
                </DialogTrigger>
